...
suspend fun <T> newSuspendedTransaction(
    ...
    withTransactionScope(context, null, db, transactionIsolation) {
        ...
        val connectionCode = tx.value.connection.hashCode()
...
suspend fun <T> continueSuspendedTransaction(context: CoroutineDispatcher? = null, db: Database? = null, transactionIsolation: Int? = null, statement: suspend Transaction.() -> T): T {
    ...
    return when (currentTransaction) {
        null -> newSuspendedTransaction(context, db, transactionIsolation, statement)
