...
suspend fun <T> continueSuspendedTransaction(context: CoroutineDispatcher? = null, db: Database? = null, statement: suspend Transaction.() -> T): T {
    ...
    return when (currentTransaction) {
        null -> newSuspendedTransaction(context = context, db = db, transactionIsolation = null, statement = statement)
        else -> currentTransaction.value.suspendedTransaction(context, statement)
    }
}
